...
def get_transforms(config: Dict[str, Any], is_train: bool = True):
    ...
    if aug_mode == 'hard':
        ...
        # 隨機裁切 vs 直接 Resize
        random_crop_scale = aug_params.get('random_crop_scale')
        if random_crop_scale:
            transform_list.append(
        ...
        # 水平翻轉
        h_flip = aug_params.get('horizontal_flip', 0.5)
        if h_flip > 0:
            transform_list.append(transforms.RandomHorizontalFlip(p=h_flip))
        
        # 垂直翻轉
        v_flip = aug_params.get('vertical_flip', 0)
        if v_flip > 0:
            transform_list.append(transforms.RandomVerticalFlip(p=v_flip))
    ...
    # Random Erasing (只在標準模式使用)
    if aug_mode != 'hard':
        random_erasing = aug_params.get('random_erasing', 0)
        if random_erasing > 0:
